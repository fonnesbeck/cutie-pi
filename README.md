...
|-----------|-------------|
| [`code-simplifier`](extensions/code-simplifier.ts) | Tracks files modified during an agent run and can automatically, or on-demand via /simplify, trigger a follow-up simplification turn. This is an extension command/follow-up turn, not a task subagent. |
...
| [`code-simplifier`](skills/code-simplifier/SKILL.md) | Instruction text injected during a simplification turn by the code-simplifier extension; invoke through /simplify or apply inline, not as a task subagent. |
...
| [`implementation-loop`](skills/implementation-loop/SKILL.md) | Bounded wrapper that drives an initial implementation from a plan, runs `review-implementation` in a fresh reviewer subagent, addresses findings with the `address-review` disposition pattern, repeats until no actionable issues remain, then runs an inline simplification pass using the code-simplifier instructions. |
| [`review-implementation`](skills/review-implementation/SKILL.md) | Compares a finished implementation against its original plan to identify gaps, divergences, simplification opportunities, and efficiency opportunities. Invoke with "compare this to the plan," "review implementation," or `/skill:review-implementation`. Produces `IMPLEMENTATION_REVIEW.md`. |
| [`review-loop`](skills/review-loop/SKILL.md) | Bounded wrapper that drives `review-plans` (a fresh independent reviewer subagent per pass) and the `address-review` triage pattern until a plan is clean. Invoke with "run a review loop," "review and address until clean," "spawn a reviewer and integrate comments," or "repeat review until all concerns are resolved." |
